...
## 七、代码运行方式说明
进入Github界面后点击最终展示小学期文件夹<br/>
![a4c5dd5a7352cf7919ba0daabcbfbfe](https://github.com/kukukukujiedao/machine-learning-group5/assets/143621522/fca6bf52-76eb-4f95-9c5c-2ddad7cf8ae0)<br/>
找到py.py文件运行即可<br/>
![76b13c664978f3a843a7d30ba1e6575](https://github.com/kukukukujiedao/machine-learning-group5/assets/143621522/5cffbd9d-e801-4ecc-9be1-b93e24e95265)

## 八、代码运行截图
运行前端界面展示：<br/>
![image](https://github.com/kukukukujiedao/machine-learning-group5/assets/143621522/f29c0b4e-449c-4503-b171-71b3ec50bc3e)<br/>
选择参数后执行界面展示：<br/>
![image](https://github.com/kukukukujiedao/machine-learning-group5/assets/143621522/9c137be0-af1c-4c73-a79f-82ae2e6be357)<br/>

## 九、每位组员的收获与感悟
### 1. 连航
这次小学期是一次综合能力的锻炼。首先在前端环节，温习了之前所学的前端知识，结合网上优秀的组件展示，构成了机器学习实践前端界面，在前后端通信环节，自己通过搜索资料了解到了eel这个python库，并在简单测试后决定用此完成前后端页面的互通。
后端方面我虽然没有参与代码编写，但整体的架构设计也让我收获了对算法模型的理解和项目设计时的逻辑思考能力。总的来说，这次项目不仅让我有了丰富的代码编写经验，还加深了我对机器学习的理解，更让我收获了新的知识，让我明白条条大路通罗马。除此之外作为组长，这次实践让我感受到了团队的魅力，每个人都要发挥自己的作用才能完成一个完整的项目。
### 2. 薛泰阳
在这个项目中，我主要实现了后端代码实现。在这个过程中，我学到了以下几点：<br/>
使用sklearn库进行机器学习任务非常方便，它提供了许多预处理、模型选择和评估的功能。例如，在这里我们使用了sklearn.datasets中的两个内置数据集Iris和Wine，以及sklearn.model_selection中的KFold和train_test_split函数。<br/>
对于分类问题，我们可以使用准确率accuracy_score作为评估指标;对于回归问题，可以使用均方误差MSE或者平均绝对误差MAE等。在实际应用中，需要根据具体问题来选择合适的评估指标。<br/>
在实现K折交叉验证时，我们首先将数据划分为k个子集，然后对每个子集进行训练和测试。这种方法可以有效地减少过拟合的风险，同时也能更好地利用有限的计算资源。然而，由于每次迭代都会重新划分数据，因此K折交叉验证的时间复杂度较高。<br/>
随机划分数据是一种更高效的方法，它可以在一次迭代中完成所有训练和测试任务。通过设置不同的测试比例，我们可以得到不同精度的估计结果。但是，如果测试比例过小，可能会导致欠拟合现象。<br/>
最后，我还学会了一些关于Matplotlib和FontPorperties的知识，它们可以帮助我们在Python中绘制图表和调整字体样式。这些工具在可视化分析和展示我们的研究成果方面非常有用。
### 3. 蒋世良宸
首先，我在编写后端代码时，需要对机器学习算法有深入的理解。这意味着我需要了解各种机器学习算法的基本原理、优缺点以及适用场景。此外，我还需要掌握一些常用的机器学习库和框架，以便在编写代码时能够快速地实现各种功能。
其次，在编写后端代码的过程中，我还需要关注数据处理相关知识。这包括如何清洗和预处理数据、如何提取有用的特征以及如何对数据进行归一化和标准化等。这些步骤对于提高模型的性能至关重要，也是我在编写代码时需要重点关注的环节。
此外，我还深刻体验到了模型部署和监控的重要性。在完成模型训练之后，我们需要将其部署到生产环境中，以便为前端用户提供服务。这就要求我们在编写后端代码时，要考虑到系统的可扩展性和高可用性。同时，我们还需要对模型的性能进行实时监控，以便及时发现问题并进行优化。
### 4. 殷泽楷
在本次的课程学习中，我学会了Python机器学习后端程序的设计思路和主要模块以及几种主要的机器学习模型的理论基础和实现方式，了解了在机器学习在实际应用中的使用方式。同时在面对不了解的问题时，通过网络或同学之间的交流解决了相关的问题，以上都是我在本次课程中的收获和感悟
